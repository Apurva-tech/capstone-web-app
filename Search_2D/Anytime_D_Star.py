...
class ADStar:
    def __init__(self, s_start, s_goal, eps, heuristic_type, env):
        self.s_start, self.s_goal = s_start, s_goal
        self.heuristic_type = heuristic_type

        self.Env = env.Env()  # class Env
        self.Plot = plotting.Plotting(s_start, s_goal, env)
        ...
        self.obs_remove = set()
        small_changes = False
        self.title =  "Anytime D*: Small changes" if small_changes else  "Anytime D*: Signficant changes" # Significant changes
        self.fig = plt.figure()
    ...
    def run(self):
        ...
        successMessage = ' ⌛ Time of execution of Anytime D* algorithm: ' + \
        ...
        if x < 0 or x > self.x - 1 or y < 0 or y > self.y - 1:
            ...
            if self.title == "Anytime D*: Small changes":
                ...
                while True:
                    ...
                    self.ComputeOrImprovePath()
                    self.plot_visited()
                    print("Path length for ARA* algorithm is: ", len(self.extract_path()))
                    self.plot_path(self.extract_path())
                    # plt.plot(self.title)
                    self.visited = set()
    ...
    def plot_visited(self):
        ...
        for x in self.visited: 
            plt.plot(x[0], x[1], marker='x', color=color[self.count])
