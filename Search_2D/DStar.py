...
class DStar:
    def __init__(self, s_start, s_goal, env):
        ...
        self.Env = env.Env()
        self.Plot = plotting.Plotting(self.s_start, self.s_goal, env)
    ...
    def on_press(self, event):
        ...
        if x < 0 or x > self.x - 1 or y < 0 or y > self.y - 1:
            ...
            if (x, y) not in self.obs:
                ...
                print("Path length for Dynamic A* (D*) algorithm is: ", len(self.path))
    ...
    def process_state(self):
        ...
        k_old = self.get_k_min()  # record (min path cost)
        self.delete(s)  # move state the min k value of this iteration s from OPEN set to CLOSED set
